...
impl Parser {
    ...
    fn parse_statment(&mut self) -> ASTStatement {
    ...
    fn parse_let_statement(&mut self) -> ASTStatement {
        self.consumer_and_check(TokenKind::Let);
        let identifier = self.consumer_and_check(TokenKind::Identifier).clone();
        self.consumer_and_check(TokenKind::Equals);
    ...
    fn parse_primary_expression(&mut self) -> ASTExpression {
        ...
        return match token.kind {
            ...
            TokenKind::LeftParen => {
                let expr = self.parse_expression();
                self.consumer_and_check(TokenKind::RightParen);
    ...
    fn consumer_and_check(&self, kind: TokenKind) -> &Token {
        let token = self.consume();
        if token.kind != kind {
            self.diagnostics_bag
                .borrow_mut()
